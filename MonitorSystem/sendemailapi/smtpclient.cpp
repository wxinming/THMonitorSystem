...
SmtpClient::SmtpClient(const QString & host, ushort port, ConnectionType connectionType) :
    name("localhost"),
...
void SmtpClient::setPort(ushort port)
{
    this->port = port;
}
...
bool SmtpClient::connectToHost()
{
    ...
    try
    {
        ...
        if (connectionType == TlsConnection) {
            ...
            if (!static_cast<QSslSocket *>(socket)->waitForEncrypted(connectionTimeout)) {
                qDebug() << static_cast<QSslSocket *>(socket)->errorString();
                emit smtpError(ConnectionTimeoutError);
                return false;
            }
